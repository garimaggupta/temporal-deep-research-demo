...
• downloading Markdown/PDF outputs once research completes
"""
...
def render_result_section() -> None:
    ...
    if result.pdf_file_path and Path(result.pdf_file_path).exists():
        with open(result.pdf_file_path, "rb") as pdf_file:
            download_cols[1].download_button(
                "📄 Download PDF",
                data=pdf_file.read(),
                file_name=Path(result.pdf_file_path).name,
                mime="application/pdf",
                use_container_width=True,
            )
    else:
        download_cols[1].caption("PDF not generated by workflow.")
    ...
    if result.image_file_path and Path(result.image_file_path).exists():
        ...
        download_cols[2].caption("Image not generated.")

    saved_path = st.session_state.saved_report_path
    if download_cols[3].button("💾 Save to workspace", use_container_width=True):
